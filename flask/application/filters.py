...
@app.template_filter(name='escapedouble')
def escapedouble_filter(value):
    """ 
        """
    value = value.replace('"', '\\"')
    return value
app.add_template_filter(escapedouble_filter)

@app.template_filter(name='listtojs')
def listtojs_filter(value):
    """ Take a comma separated list and turn it into something that can be 
        included in a javascript string.
        """
    try:
        value = value.replace(',', '","')
        return '"%s"' % value
        ...
        return value
app.add_template_filter(listtojs_filter)
